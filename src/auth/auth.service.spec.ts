...
describe('AuthService', () => {
  ...
  describe('register()', () => {
    ...
    it('should register a new user and return id and username', async () => {
      ...
      mockUserRepository.createUser.mockResolvedValue({ id: 1, username: 'john_doe' });
    ...
    it('should throw ConflictException if username already exists', async () => {
      mockUserRepository.findByUsername.mockResolvedValue({ id: 1, username: 'john_doe' });
  ...
  describe('login()', () => {
    const dto = { username: 'john_doe', password: 'Secret@123' };
    const mockUser = { id: 1, username: 'john_doe', password: 'hashed_password' };
    ...
    it('should return a JWT token on successful login', async () => {
      ...
      expect(mockUserRepository.findByUsername).toHaveBeenCalledWith('john_doe');
      expect(bcrypt.compare).toHaveBeenCalledWith('Secret@123', 'hashed_password');
  ...
  describe('updateUser()', () => {
    ...
    it('should update username successfully', async () => {
      ...
      mockUserRepository.findByUsername.mockResolvedValue(null);
      mockUserRepository.updateUser.mockResolvedValue({ id: 1, username: 'new_username' });

      const result = await service.updateUser(1, { username: 'new_username' });

      expect(mockUserRepository.findById).toHaveBeenCalledWith(1);
      expect(mockUserRepository.findByUsername).toHaveBeenCalledWith('new_username');
      expect(mockUserRepository.updateUser).toHaveBeenCalledWith(1, { username: 'new_username' });
    ...
    it('should update password successfully', async () => {
      ...
      mockUserRepository.updateUser.mockResolvedValue({ id: 1, username: 'john_doe' });

      const result = await service.updateUser(1, { password: 'NewSecret@123' });

      expect(bcrypt.hash).toHaveBeenCalledWith('NewSecret@123', 10);
      expect(mockUserRepository.updateUser).toHaveBeenCalledWith(1, { password: 'new_hashed_password' });
      expect(result).toEqual({ id: 1, username: 'john_doe' });
    });
    ...
    it('should throw NotFoundException if user does not exist', async () => {
      ...
      await expect(service.updateUser(99, { username: 'new_name' })).rejects.toThrow(NotFoundException);
      expect(mockUserRepository.updateUser).not.toHaveBeenCalled();
    });

    it('should throw ConflictException if new username is already taken', async () => {
      mockUserRepository.findById.mockResolvedValue(mockUser);
      mockUserRepository.findByUsername.mockResolvedValue({ id: 2, username: 'taken_name' });

      await expect(service.updateUser(1, { username: 'taken_name' })).rejects.toThrow(ConflictException);
      expect(mockUserRepository.updateUser).not.toHaveBeenCalled();
    });
  ...
  describe('resetPassword()', () => {
    ...
    it('should throw BadRequestException if token is expired', async () => {
      const expiredUser = { ...mockUser, resetTokenExpiry: new Date(Date.now() - 1000) };
